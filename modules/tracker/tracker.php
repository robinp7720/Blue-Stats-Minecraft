<script>
		            ...
		            jQuery.each(json.data, function(i, val) {
		                ...
		                jQuery.each(stats, function(stat, statreplace) {
		                	if (oldPlayer[stat]!=val[stat]){
		                		change = true;
		                		if (stat=="lastleave"){
		                			$( ".tracker-page" ).prepend( "<b>" + val.name + "</b> Disconnected<br>");
		                			oldPlayers[val.name][stat]=val[stat];
		                		}else if(stat=="lastjoin"){
		                			$( ".tracker-page" ).prepend( "<b>" + val.name + "</b> Joined<br>");
		                			oldPlayers[val.name][stat]=val[stat];
		                		}else{
			                		...
			                		console.log(diffrence);
			                   		$( ".tracker-page" ).prepend(string+"<br>");
			                   		oldPlayers[val.name][stat]=val[stat];
		                   		}
		               		}
		                });
		...
		<div class="tracker-page noscrollbars" style="width:100%; height: calc(100vh - 84px); overflow:scroll;">
